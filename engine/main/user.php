...
class User {
	...
	private $test_server;
	private $user_vk_id;
	private $google_auth_check;
    private $user_google_auth;
  	...
  	public function __construct($registry) {
		...
		if(isset($this->registry->cookie->data['uid'])) {
			...
			if ($query->num_rows) {
				$this->google_auth_check = $query->row['google_auth_check'];
                $this->user_google_auth = $query->row['user_google_auth'];
				...
				$this->themeid = $query->row['themeid'];
	...
	public function isLogged() {
  	...
  	public function logout() {
		...
		$this->google_auth_check = null;	
        $this->user_google_auth = null;
	}
	
	public function getGoogle_auth_check() {
	    if(isset($this->google_auth_check)) {
		    return $this->google_auth_check;
	    } else {
		    return 0;
	    }
    }
	
    public function getGoogle_auth() {
	    if(isset($this->user_google_auth)) {
		    return $this->user_google_auth;
	    } else {
		    return 0;
	    }
    }
