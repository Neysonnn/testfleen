...
class usersModel extends Model {
	...
	public function deleteAuth($authid) {
		...
		public function updateAuth($data = array()) {
		$sql = "UPDATE `users_auth`";
		if(!empty($data)) {
			$count = count($data);
			$sql .= " SET";
		...
		$sql .= " WHERE `auth_key` = '" . (string) $this->db->escape(@$this->cookie->data['uid']) . "'";
		$query = $this->db->query($sql);
		return true;
	}
	...
	public function getSkidkaBySCode($code, $rewrite) {
		$sql = "SELECT * FROM `codes` WHERE `cod` = '" . $this->db->escape($code) . "' AND `used` < `uses` LIMIT 1";
		$query = $this->db->query($sql);
		
		if($rewrite == true){
			$zapros = $this->db->query("UPDATE `codes` SET `used`= `used`+1 WHERE `cod` = '" . $this->db->escape($code) . "' AND `used` < `uses` LIMIT 1");
		}
		return $query->row;
	}
	...
	public function getUserFs($userid) {
		$sql = "SELECT * FROM `users_fs` WHERE `user_id` = '" . (int)$userid . "' ";
		$query = $this->db->query($sql);
		return $query->rows;
	}

	public function addFs($data) {
		$sql = "INSERT INTO `users_fs` SET ";
		$sql .= "user_id = '" . (int)$data['user_id'] . "', ";
		$sql .= "fs_id = '" . (int)$data['fs_id'] . "'";
		$this->db->query($sql);
		return $this->db->getLastId();
	}
	...
	public function createAuthLog($userid, $ip, $status, $password) {
		...
		$ipDetail['countryCode'] = $countryCode[1];
	  	...
	  	$query=$this->db->query("INSERT INTO `authlog` (`id`, `user`, `ip`, `city`, `country`, `code`, `datetime`, `status`, `password`) VALUES (NULL, '".$userid."', '".$ip."', '".$ipDetail['city']."', '".$ipDetail['country']."', '".$ipDetail['countryCode']."', NOW(), '".$status."', '".md5($password)."');");
	}
	...
	public function sendAdmInfo($message) {	
