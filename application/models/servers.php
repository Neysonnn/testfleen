...
class serversModel extends Model {
	...
	/* ======================================================= */
		/* ===================== Autoinstall filterscripts ===================== */
public function createFs($data) {
	$sql = "INSERT INTO `servers_fs` SET ";		
	$sql .= "`game_id` = '" . $data['game_id'] . "', ";
	$sql .= "`fs_url` = '" . $data['fs_url'] . "', ";
	$sql .= "`fs_name` = '" . $data['fs_name'] . "', ";
	$sql .= "`fs_status` = '" . (int)$data['fs_status'] . "', ";
	$sql .= "`fs_textx` = '" . strip_tags(htmlspecialchars_decode($this->db->escape($data['fs_textx'])), '<img><span><ul><ol><pre><li><div><em><strong><sup><code>') . "', ";
	$sql .= "`fs_img` = '" . $data['fs_img'] . "', ";	
	$sql .= "`fs_price` = '" . $data['fs_price'] . "', ";
	$sql .= "`fs_id` = '" . $data['fs_id'] . "', ";
	$sql .= "`fs_arch` = '" . $data['fs_arch'] . "', ";
	$sql .= "`fs_cfg` = '" . $data['fs_cfg'] . "' ";
	$this->db->query($sql);
	$return=$this->db->getLastId();		
	return $return;
}
	public function deleteFs($fsid) {
		$sql = "DELETE FROM `servers_fs` WHERE fs_id = '" . (int)$fsid . "'";
		$this->db->query($sql);
	}
	
	public function updateFs($fsid, $data = array()) {
		$sql = "UPDATE `servers_fs`";
		if(!empty($data)) {
			$count = count($data);
			$sql .= " SET";
			foreach($data as $key => $value) {
		...
		$sql .= " WHERE `fs_id` = '" . (int)$fsid . "'";
		$query = $this->db->query($sql);
	}
	public function getFs($data = array(), $joins = array(),$sort = array(), $options = array()) {
		$sql = "SELECT * FROM `servers_fs`";
		foreach($joins as $join) {
			$sql .= " LEFT JOIN $join";
			switch($join) {
				case "games":
					$sql .= " ON servers_fs.fs_id=games.game_id";
					break;
			}
		}
		if(!empty($data)) {
	...
	public function getFsById($fsid, $joins = array()) {
		$sql = "SELECT * FROM `servers_fs`";
		foreach($joins as $join) {
			$sql .= " LEFT JOIN $join";
			switch($join) {
				case "games":
					$sql .= " ON servers_fs.game_id=games.game_id";
					break;
			}
		}
		$sql .=  " WHERE `fs_id` = '" . (int)$fsid . "' LIMIT 1";
		$query = $this->db->query($sql);
		return $query->row;
	}
	
	public function getTotalFs($data = array()) {
		$sql = "SELECT COUNT(*) AS count FROM `servers_fs`";
		if(!empty($data)) {
			$count = count($data);
			$sql .= " WHERE";
			foreach($data as $key => $value) {
