...
class controlController extends Controller {
	...
	public function settingFive($serverid = null) {
		if(!$this->user->isLogged()) {
			$this->session->data['error'] = "Вы не авторизированы!";
			$this->response->redirect($this->config->url . 'news/index');
		}
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
			if($server['server_status'] == 1){	
				if($server) {
					$server_port = @$this->request->post['server_port'];
					if(mb_strlen($server_port) < 4 || mb_strlen($server_port) > 18) {
						$this->data["status"] = "error";
						$this->data["error"] = "Порт должен состоять от 4-х цифр";
						return json_encode($this->data);	
					}
					$this->serversModel->updateServer($serverid, array('server_port' => $server_port));
					
					$user_id = @$this->request->post['user_id'];
					$this->serversModel->updateServer($serverid, array('user_id' => $user_id));
					
					$server_date_end = @$this->request->post['server_date_end'];
					$this->serversModel->updateServer($serverid, array('server_date_end' => $server_date_end));
					
					$server_slots = @$this->request->post['server_slots'];
					$this->serversModel->updateServer($serverid, array('server_slots' => $server_slots));
					
					$game_id = @$this->request->post['game_id'];
					$this->serversModel->updateServer($serverid, array('game_id' => $game_id));
