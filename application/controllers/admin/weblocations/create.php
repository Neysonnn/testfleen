...
class createController extends Controller {
	...
	public function ajax() {
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(!$errorPOST) {
				...
				$reseller_status = @$this->request->post['reseller_status'];
				$user = @$this->request->post['user'];
				$password = @$this->request->post['password'];
				$password2 = @$this->request->post['password2'];
				$status = @$this->request->post['status'];
				$panel = @$this->request->post['panel'];
				...
				$locationData = array(
					...
					'location_panel'		=> (int)$panel
				);
				
				if($panel == 1) {
					$locationData['location_reseller_status'] = $reseller_status;
				}
	...
	private function validatePOST() {
		...
		elseif($panel == 1) {
			if($reseller_status < 0 || $reseller_status > 1) {
				$result = "Укажите допустимый статус реселлера!";
			}
		}
