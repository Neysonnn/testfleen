...
class orderController extends Controller {
	...
	public function ajax() {
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(!$errorPOST) {
				...
				$test_vk = $this->user->getUser_vk_id();
				...
				if($port) {
					...
					if($game['game_id'] == 1 OR $game['game_id'] == 2) {
						if($this->config->serv_test == 1) {
							if($test_server == 1){
								...
								$wasteData = array(
								  ...
								  'waste_ammount'	=> 0,
								  'waste_status'	=> 1,
								  'waste_usluga'	=> "Заказ бесплатного сервера gs$serverid"
								); 
								...
								$this->data['status'] = "error";
								$this->data['error'] = "Для заказа бесплатного сервера у Вас должен быть привязан ВКонтакте!";
							}
							} else {
								$this->data['status'] = "error";
								$this->data['error'] = "Вы уже брали бесплатный бесплатный период, попробуйте рассмотреть платные тарифы  от 12р/мес!";
							}
						} else {
							$this->data['status'] = "error";
							$this->data['error'] = "Бесплатный тариф отключен на хостинге!";
						}
					}
					elseif($balance >= $price AND $test_periud == false) {
						...
						$this->usersModel->sendAdmInfo("[Заказ сервера]\n🕺Пользователь: $firstname $lastname (id $userid)\n💳Сумма: $price  ₽\n🔎ID сервера: $serverid");
	...
	private function validatePOST() {
		...
		elseif(!in_array($gameid, array(1,2))) {
			if($days != "30" && $days != "60" && $days != "90" && $days != "180" && $days != "360" && $days != "15" && $days != "0") {
