...
class controlController extends Controller {
	public function index($webid = null) {
		...
		public function ajax2($webid = null) {
		if(!$this->user->isLogged()) {  
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(!$errorPOST) {
				$domain = @$this->request->post['password'];
					$result = $this->webhostModel->updateDomain($webid, $domain);
						$this->data['status'] = "success";
						$this->data['success'] = "Домен был успешно изменён!";
			} else {
				$this->data['status'] = "error";
	...
	public function buy_months($webid = null) {
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if($days != "30" && $days != "60" && $days != "90" && $days != "180" && $days != "360" && $days != "15" && $days != "5475") {
				$this->data['status'] = "error";
