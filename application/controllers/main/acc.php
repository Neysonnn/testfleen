...
class accController extends Controller {
	...
	public function action($action = null) {
		...
		switch($action) {
						case 'getQR_google_auth': {
				if($this->request->server['REQUEST_METHOD'] == 'POST') {
					$user = $this->usersModel->getUserById($userid);
					if($user['user_google_auth'] != 1) {
						require_once(ENGINE_DIR . 'libs/GoogleAuthenticator/GoogleAuthenticator.php');
						$ga = new GoogleAuthenticator;
						$secret = $ga->generateSecret();
						$url = str_replace(array('http://', 'https://', '/'), '', $this->config->url);
						$this->usersModel->updateUser($userid, array('user_google_auth_key'	=> $secret));
						$this->data['status'] = "success";
						$this->data['url'] = $url;
						$this->data['secret_key'] = $secret;
						$this->data['user'] = $user['user_email'];
					} else {
						$this->data['status'] = "error";
						$this->data['error'] = "Google Authenticator уже привязан к аккаунту!";						
					}
				}
				break;
			}
			case 'set_google_auth': {
				if($this->request->server['REQUEST_METHOD'] == 'POST') {
					...
					if($user['user_google_auth'] != 1) {
						...
						$this->usersModel->updateUser($userid, array('user_google_auth'	=> 1));
						$this->data['status'] = "success";
						$this->data['success'] = "Вы успешно привязали Google Authenticator к аккаунту!";
					} else {
						$this->data['status'] = "error";
						$this->data['error'] = "Google Authenticator уже привязан к аккаунту!";						
					}
			...
			case 'unset_google_auth': {
				if($this->request->server['REQUEST_METHOD'] == 'POST') {
					...
					if($user['user_google_auth'] == 1) {
						...
						$this->usersModel->updateUser($userid, array('user_google_auth'	=> 0, 'user_google_auth_key' => null));
						$this->data['status'] = "success";
						$this->data['success'] = "Вы успешно отвязали Google Authenticator от аккаунта!";
					} else {
						$this->data['status'] = "error";
						$this->data['error'] = "Google Authenticator уже отвязан от аккаунта!";						
					}
				}
				break;
			}
