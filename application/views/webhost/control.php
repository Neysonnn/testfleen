				...
				<div class="col-xl-9">
					<div class="row">
						...
						<div class="col-xl-12">
							<div class="card card-custom mb-3">
								<div class="card-header border-0">
									<?php if($webhost['location_panel'] == 1): ?> 
									<h3 class="card-title">Адрес Fastpanel<small class="text-muted font-size-sm ml-2">https://<?php echo $webhost['location_url'] ?></small></h3>
									<div class="card-toolbar">
										<a href="https://<?php echo $webhost['location_url'] ?>" target="_blank" class="btn btn-clean btn-icon">
									...
									<?php elseif($webhost['location_panel'] == 2): ?> 
									<h3 class="card-title">Адрес ISPmanager<small class="text-muted font-size-sm ml-2">https://<?php echo $webhost['location_url'] ?></small></h3>
									<div class="card-toolbar">
										<a href="https://<?php echo $webhost['location_url'] ?>" target="_blank" class="btn btn-clean btn-icon">
										<i class="fa fa-external-link-alt" data-toggle="tooltip" data-placement="right" title="" data-original-title="Перейти в панель управления"></i>
										</a>
						...
						<div class="col-xl-6">
							<div class="card card-custom mb-3">
								<div class="card-header border-0">
									<h3 class="card-title">Домен<small class="text-muted font-size-sm ml-2"><?php echo $webhost['web_domain'] ?></small></h3>
									<div class="card-toolbar">
										<a href="javascript:;" data-toggle="modal" data-target="#domains" class="btn btn-clean btn-icon">
										<i class="fas fa-cogs" data-toggle="tooltip" data-placement="right" title="" data-original-title="Изменить"></i>
										</a>
									</div>
								</div>
...
<?php if($webhost['web_status'] == 1): ?> 
<!--begin::Modal-->
<div class="modal fade" id="domains" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Изменить домен</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
				<i aria-hidden="true" class="ki ki-close"></i>
				</button>
			</div>
			<form id="editForm2" method="POST" style="padding:0px; margin:0px;">
				<div class="modal-body">
					<div class="form-group">
						<label>Домен</label>
						<input class="form-control" id="password" name="password" placeholder="Домен" >
					</div>
				<div class="modal-footer" style="padding: 0.5rem;">
					<button type="button" class="btn btn-light-primary font-weight-bold" data-dismiss="modal">Отмена</button>
...
<?php if($webhost['web_status'] == 1): ?> 
<!--begin::Modal-->
<div class="modal fade" id="ftp_pass" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
...
<script>	
	...
	$('#editForm2').ajaxForm({ 
		url: '/webhost/control/ajax2/<?php echo $webhost['web_id'] ?>',
		dataType: 'text',
		success: function(data) {
			console.log(data);
			data = $.parseJSON(data);
