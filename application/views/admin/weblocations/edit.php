				...
				<div class="card-body">
					<form id="editForm" method="POST">
						...
						<div class="form-group form-md-line-input">
							...
							<input type="text" class="form-control" id="ns_servers" name="ns_servers" placeholder="Введите NS Сервера локации (ns1.domain.ru и ns2.domain.ru), если нет то просто нет :)" value="<?php echo $location['ns_servers'] ?>">
						</div>
						<div class="form-group form-md-line-input">
						...
						<div class="form-group form-md-line-input">
							<label>Выберите панель</label>
							<select class="form-control" id="panel" name="panel" aria-required="true" aria-invalid="false" aria-describedby="delivery-error" onChange="toggleReseller()">
								<option value="1" <?php if($location['location_panel'] == 1): ?> selected="selected"<?php endif; ?>>Fastpanel</option>
								<option value="2" <?php if($location['location_panel'] == 2): ?> selected="selected"<?php endif; ?>>ISPmanager 6 (Lite-Pro-Host)</option>
							</select>
						</div>
						<div class="form-group form-md-line-input">
							<label>Пользователь реселлер (для Fastpanel)?</label>
							<select class="form-control" id="reseller_status" name="reseller_status" aria-required="true" aria-invalid="false" aria-describedby="delivery-error">
								<option value="1"<?php if($location['location_reseller_status'] == 1): ?> selected="selected"<?php endif; ?>>Да</option>
								<option value="0"<?php if($location['location_reseller_status'] == 0): ?> selected="selected"<?php endif; ?>>Нет</option>
							</select>
						</div>
						<div class="form-group form-md-line-input">
...
<script>
	...
	function toggleReseller() {
		...
		if(panel == 1) {
			$('#reseller_status').prop('disabled', false);
		} else {
			$('#reseller_status').prop('disabled', true);
		}
	}
</script>
<?php echo $footer ?>